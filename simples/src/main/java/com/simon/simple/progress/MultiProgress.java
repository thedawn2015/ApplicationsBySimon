...
public class MultiProgress extends View {
    ...
    public MultiProgress(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MultiProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;

        TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.MultiProgress);
        nodesNum = mTypedArray.getInteger(R.styleable.MultiProgress_nodesNum, 1); //默认一个节点
        nodeRadius = mTypedArray.getDimensionPixelSize(R.styleable.MultiProgress_nodeRadius, 10); //节点半径
        progressingDrawable = mTypedArray.getDrawable(R.styleable.MultiProgress_progressingDrawable);
        unprogressingDrawable = mTypedArray.getDrawable(R.styleable.MultiProgress_unprogressingDrawable);
        progresFailDrawable = mTypedArray.getDrawable(R.styleable.MultiProgress_progresFailDrawable);
        progresSuccDrawable = mTypedArray.getDrawable(R.styleable.MultiProgress_progresSuccDrawable);
        processingLineColor = mTypedArray.getColor(R.styleable.MultiProgress_processingLineColor, DEFAULT_LINE_COLOR);
        currNodeState = mTypedArray.getInt(R.styleable.MultiProgress_currNodeState, 1);
        currNodeNO = mTypedArray.getInt(R.styleable.MultiProgress_currNodeNO, 1);
