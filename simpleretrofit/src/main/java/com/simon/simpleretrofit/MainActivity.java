...
import com.simon.simpleretrofit.okhttpdownload.ProgressDownloader;
import com.simon.simpleretrofit.okhttpdownload.ProgressListener;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, ProgressListener {
    ...
    Button simple_download;
    Button simple_pause;
    Button simple_continue;
...
//    String url = "http://apk.hiapk.com/appdown/com.tencent.qqpimsecure?em=5&p=android&f_name=%E6%89%8B%E6%9C%BA%E7%AE%A1%E5%AE%B6";
    ...
    private void assignViews() {
        ...
        simple_download = (Button) findViewById(R.id.simple_download);
        simple_pause = (Button) findViewById(R.id.simple_pause);
        simple_continue = (Button) findViewById(R.id.simple_continue);
        ...
        simple_download.setOnClickListener(MainActivity.this);
        simple_pause.setOnClickListener(MainActivity.this);
        simple_continue.setOnClickListener(MainActivity.this);
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.simple_rxjava:
                ...
                break;
            case R.id.simple_download:
                // 新下载前清空断点信息
                breakPoints = 0L;
                file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "sample.apk");
                downloader = new ProgressDownloader(url, file, MainActivity.this);
                downloader.download(0L);
                break;
            case R.id.simple_pause:
                downloader.pause();
                Toast.makeText(this, "下载暂停", Toast.LENGTH_SHORT).show();
                // 存储此时的totalBytes，即断点位置。
                breakPoints = totalBytes;
                break;
            case R.id.simple_continue:
    ...
    public void update(final long bytesRead, final long contentLength, boolean done) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                //                                        download_progress.setProgress(transferProgress(currentSize, totalSize));
                downloaded_size.setText("当前下载量为：" + bytesRead + "\n总量为：" + contentLength);
            }
        });
